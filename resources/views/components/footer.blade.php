...
$aboutText = \App\Modules\Setting\Models\Setting::where('key', 'footer_about_text')->value('value');
@endphp
            ...
            @unless ($minimal)
                <nav class="flex flex-wrap gap-4 text-sm text-ink/70">
                    @foreach ($footerLinks as $link)
                        <a href="{{ $link['url'] }}" wire:navigate class="hover:text-brand-primary">{{ $link['label'] }}</a>
                    @endforeach
                </nav>
