...
3. **CLI (same pipeline)** when you want a packet on disk or in a script:
   ...
   `python ".claude/skills/token-reducer/scripts/context_pipeline.py" run --inputs ./src --query "Locate JWT validation" --top-k 3`
...
- End-to-end run (defaults from plugin `settings.json`; override flags as needed):
  ...
  `python ".claude/skills/token-reducer/scripts/context_pipeline.py" run --inputs . --query "${ARGUMENTS}" --hybrid-mode fallback --top-k 3`

- Self-test:

  `python ".claude/skills/token-reducer/scripts/context_pipeline.py" self-test`
